...
if RANDOM_AND_ITERATIVE:
    split_and_save(xfiles, ylabels, 0.2, "split2-random", random_state=2137, iterative=False, merge_train_val=False)
    pd.DataFrame(mlb.classes_)[0].to_json('split2-random/labels.json', orient='values')

    split_and_save(xfiles, ylabels, 0.2, "split2-iterative", random_state=2137, iterative=True, merge_train_val=False)
    pd.DataFrame(mlb.classes_)[0].to_json('split2-iterative/labels.json', orient='values')

    split_and_save(xfiles, ylabels, 0.2, "split3-random", random_state=2137, iterative=False, merge_train_val=True)
    pd.DataFrame(mlb.classes_)[0].to_json('split3-random/labels.json', orient='values')

    split_and_save(xfiles, ylabels, 0.2, "split3-iterative", random_state=2137, iterative=True, merge_train_val=True)
    pd.DataFrame(mlb.classes_)[0].to_json('split3-iterative/labels.json', orient='values')
else:
    split_and_save(xfiles, ylabels, 0.2, "split2", random_state=2137, merge_train_val=False)
    pd.DataFrame(mlb.classes_)[0].to_json('split2/labels.json', orient='values')

    split_and_save(xfiles, ylabels, 0.2, "split3", random_state=2137, merge_train_val=True)
    pd.DataFrame(mlb.classes_)[0].to_json('split3/labels.json', orient='values')
